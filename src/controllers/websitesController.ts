...
export default class WebsiteController {
  ...
  async create(req: Request, res: Response) {
    ...
    const newWebsite = await websiteUseCase.create({ name, url, adminId: Number(adminId) } as IWebsite)
  ...
  async update(req: Request, res: Response) {
    const { id } = req.params
    const { name, url, adminId } = req.body
    const website = await websiteUseCase.update(Number(id), { name, url: url, adminId: Number(adminId) } as IWebsite)
    return res.status(200).json(website)
  }
